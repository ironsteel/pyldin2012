...
short	SDRAMSIM::operator()(int clk, int cke, int cs_n, int ras_n, int cas_n, int we_n,
	...
	if (m_pwrup < POWERED_UP_STATE) {
		...
		if (m_pwrup == 0) {
			...
			} else assert((!cs_n)&&(!ras_n)&&(!cas_n)&&(we_n));
		} else if (m_pwrup == 3) {
			const int tRSC = 2;
		...
		if ((m_clocks_till_idle > 0)&&(m_next_wr)) {
			//printf("m_next_wr: SDRAM[%08x] <= %04x\n", m_wr_addr, data & 0x0ffff);
                        //if ((data & 0x0ffff) != 0)
                         //   printf("m_next_wr: SDRAM[%08x:%08x] <= %04x\n", m_wr_addr, addr, data & 0x0ffff);
